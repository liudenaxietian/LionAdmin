 ...
 * 返回所有子路由
 */
const getChildrenRoutes = (routes: RouteRecordNormalized[]) => {
  const result: any[] = []
 ...
 */
export function generateMenus(routes: RouteRecord[], basePath = '') {
  const result: any[] = []
  // 遍历路由表
  routes.forEach((item: RouteRecordNormalized) => {
